...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete p2p database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no p2p datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old p2p datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected p2p 0.7 datadir."
    ;;
  3)
    echo "Detected p2p pre-0.8 datadir."
    ;;
  4)
    echo "Detected p2p 0.8 datadir."
    ;;
esac
