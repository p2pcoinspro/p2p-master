...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("p2pMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("p2pMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("p2pMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("p2p-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running p2pMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
