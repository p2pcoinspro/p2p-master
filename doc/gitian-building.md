...
*Setup instructions for a gitian build of p2p using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the p2p
...
Clone the git repositories for p2p and gitian and then checkout the p2p version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the p2p repository
...
To build p2p (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the p2p repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/p2p/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/p2papp/p2pcoin
...
For example:
```bash
URL=https://github.com/konez2k/p2p.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit p2pcoin=${COMMIT} --url p2pcoin=${URL} ../p2pcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit p2pcoin=${COMMIT} --url p2pcoin=${URL} ../p2pcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit p2pcoin=${COMMIT} --url p2pcoin=${URL} ../p2pcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/p2p-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/p2p-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/p2p-build.assert
```
...
[p2p/gitian.sigs](https://github.com/p2papp/gitian.sigs/) repository, or if that's not possible to create a pull
request.
