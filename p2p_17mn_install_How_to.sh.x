...
NODEIPV6=$(curl api6.ipify.org) >/dev/null 2>&1
...
echo '  ip=`cat /root/.p2p_$i/p2p.conf |sed "/rpcbind/d" | grep bind | sed 's/bind=//g'`' >> /usr/local/bin/p2pinfo
echo '  echo "mn$i    $ip     $INFO   $mnstatus" >> p2pinfo' >> /usr/local/bin/p2pinfo
